...
export function RoutesBar(props: IRoutesBarProperties) {
  ...
  return (
    <div className="flex flex-col h-fit items-center max-md:border max-md:border-t-2 max-md:border-white/20 max-md:left-0 z-50 md:sticky fixed md:top-4 max-md:w-screen max-md:bottom-0">
      ...
      <Paper className="md:rounded-[2rem] justify-between md:w-60 max-md:w-full flex md:flex-col">
        {ROUTES.map((route, index) => {
          return (
            <Link
              className={clsx(
                "flex font-rounded rounded-full text-base items-center p-2 font-semibold mr-2 md:hover:bg-black/5",
                {
